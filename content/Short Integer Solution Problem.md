The **Short Integer Solution (SIS) problem** is a mathematically hard problem in lattice-based cryptography. Introduced by Miklós Ajtai in 1996. It is prominent in post-quantum cryptography stuff.
...
## Importance
If there were no length restriction on $x$, solving the equation $Ax \equiv 0 \pmod q$ would be trivial using standard linear algebra (like Gaussian elimination). The requirement that the vector $x$ must be composed of *short* integers is what makes the problem hard. 
